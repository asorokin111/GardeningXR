...
public class SceneTutorial : MonoBehaviour
{
    ...
    IEnumerator PlayTutorial()
    {
        ...
        for (int i = 0; i < phrasesCount; i++)
        {
            ...
            _text.text = "";
            WaitForSeconds waitUntilNextSimbol = new WaitForSeconds(nextPhrase.phraseTypingDuraction / nextPhrase.text.Length);

            for (int j = 0; j < nextPhrase.text.Length; j++)
            {
                yield return waitUntilNextSimbol;
                _text.text += nextPhrase.text[j];
            }
